...
public:
    ...
    using value_type = std::variant<std::reference_wrapper<const MsgFoo>, std::reference_wrapper<const MsgBar>, std::reference_wrapper<const MsgBaz>>;

    msg_stream_iter(const void *ptr, std::size_t size) : _current(static_cast<const std::uint8_t *>(ptr)), _size(size) {
    }
    ...
    msg_stream_iter operator++(int) const {
        return std::visit([this](const auto &msg) {
            const auto header_and_msg_size = sizeof(Type) + msg_size(msg.get());
            return msg_stream_iter(_current + header_and_msg_size, _size - header_and_msg_size);
        },
                          _get());
    }

    msg_stream_iter &operator++() {
        return *this = (*this)++;
    }
